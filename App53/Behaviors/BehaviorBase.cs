...
namespace App53.Behaviors
{
    ...
    /// If you are planning on using DependencyCollection as a dependency property on your custom behavior,
    /// Please re-instantiate the DependecyCollection in a parameterless CTOR of your custom behavion.
    /// </summary>
        ...
        where T : FrameworkElement
    {
        ...
        protected static bool IsBehaviorActive(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var behavior = d as BehaviorBase<T>;
            if (behavior != null && behavior.IsActive)
                return true;
            return false;
        }

        protected static B GetBehavior<B>(DependencyObject d)
            where B : BehaviorBase<T>
        {
            var behavior = d as B;
            if (behavior != null && behavior.IsActive)
                return behavior;
