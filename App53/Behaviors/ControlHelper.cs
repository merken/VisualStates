...
namespace App53.Behaviors
{
    ...
    public static class ControlHelper
    {
        ...
        public static object FindParent(Type t, DependencyObject child, Func<object, bool> filter = null)
        {
            FrameworkElement parent = VisualTreeHelper.GetParent(child) as FrameworkElement;
            while (parent != null)
            {
                try
                {
                    if (parent.GetType().Equals(t) || parent.GetType().GetTypeInfo().IsSubclassOf(t))
                    {
                        var correctlyTyped = Convert.ChangeType(parent, t);
                        if (correctlyTyped != null && (filter == null || filter(correctlyTyped)))
                            return correctlyTyped;
                    }
                }
